...
results_summarized_short = filter(results_summarized, spatial_scale %in% c(1,5,10,20))
...
ggplot(results_summarized_short, aes(x=temporal_scale, y=1-mse))+
  ...
  theme(plot.subtitle = element_text(vjust = 1), 
        ...
        panel.background = element_rect(fill = "gray97")) + theme(legend.position = "none") +labs(x = "Temporal Grain", y = "1 - MSE")
...
ggplot(results_summarized_short, aes(x=spatial_scale, y=1-mse))+
  ...
  theme(plot.subtitle = element_text(vjust = 1), 
        plot.caption = element_text(vjust = 1), 
        panel.background = element_rect(fill = "gray97")) + theme(legend.position = "none") +labs(x = "Spatial Grain", y = "1 - MSE")
